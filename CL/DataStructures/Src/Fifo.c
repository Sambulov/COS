...
typedef struct FIFO_Internal_T {
	...
	uint8_t reserved;
	void *buffer[2];
	struct FIFO_Internal_T *redirectLink;
} _Fifo_t;
...
static uint8_t _bFifoDoRedirection(_Fifo_t **desc) {
	uint8_t res = 0;
	if (desc != libNULL) {
		_Fifo_t *nextDesc = *desc;
		while (bFifoIsValid((Fifo_t *)nextDesc)) {
			res = 1;
			*desc = nextDesc;
			nextDesc = nextDesc->redirectLink;
		}
	}
	return res;
}
...
uint8_t bFifoInit(Fifo_t *pxFifo, uint8_t *pucBuffer, uint16_t uSize, uint8_t isDoubleBufferization) {
	if ((pxFifo == libNULL) || (pucBuffer == libNULL)) 
	  return false;
	_Fifo_t *pxDescriptor = (_Fifo_t *)pxFifo;
	pxDescriptor->validation = FIFO_VALIDATION_MARKER;
	pxDescriptor->isDouble = 0;
	pxDescriptor->redirectLink = libNULL;
	pxDescriptor->isInTransaction = 0;
	pxDescriptor->isDouble = (isDoubleBufferization != 0);
	pxDescriptor->rdBufIndex = 0;
	uint16_t size = (uSize - sizeof(Fifo_t));
	pucBuffer += sizeof(Fifo_t);
	if (isDoubleBufferization) {
		size >>= 1;
		pxDescriptor->buffer[1] = pxDescriptor->pxIface->pfBufferInit(pucBuffer + size, size);
		if (pxDescriptor->buffer[1] == libNULL) {
			return false;
		}
	}
	pxDescriptor->buffer[0] = pxDescriptor->pxIface->pfBufferInit(pucBuffer, size);
...
int32_t lFifoAvailableToRead(Fifo_t *pxDescriptor) {
	if (_bFifoDoRedirection((_Fifo_t **)&pxDescriptor)) {
...
uint8_t bFifoRedirect(Fifo_t *pxFrom, Fifo_t *pxTo) {
	uint8_t res = 0;
	_Fifo_t *from = (_Fifo_t *)pxFrom;
	if (bFifoIsValid(pxFrom)) {
		if (bFifoIsValid(pxTo) && (pxFrom != pxTo)) {
			res = 1;
			from->redirectLink = (_Fifo_t *)pxTo;
		}
		else if (pxTo == libNULL) {
			res = 1;
			from->redirectLink = libNULL;
		}
	}
	return res;
}
...
uint8_t bFifoTransactionBegin(Fifo_t *pxDescriptor) {
	_Fifo_t *desc = (_Fifo_t *)pxDescriptor;
	if (_bFifoDoRedirection(&desc) && (desc->pxIfaceEx != libNULL)) {
...
uint8_t fifo_init(Fifo_t *pxFifo, uint8_t *buffer, uint16_t size, uint8_t double_bufferization) __attribute__ ((alias ("bFifoInit")));
uint8_t fifo_is_valid(fifo_t *fifo) __attribute__ ((alias ("bFifoIsValid")));
int32_t fifo_available_to_read(fifo_t *fifo) __attribute__ ((alias ("lFifoAvailableToRead")));
int32_t fifo_available_to_write(fifo_t *fifo) __attribute__ ((alias ("lFifoAvailableToWrite")));
int32_t fifo_write(fifo_t *fifo, const uint8_t *data, uint16_t count) __attribute__ ((alias ("lFifoWrite")));
int32_t fifo_write_all(fifo_t *fifo, const uint8_t *data, uint16_t count) __attribute__ ((alias ("lFifoWriteAll")));
int32_t fifo_fill(fifo_t *fifo, uint8_t filler, uint16_t count) __attribute__ ((alias ("lFifoFill")));
int32_t fifo_fill_all(fifo_t *fifo, uint8_t filler, uint16_t count) __attribute__ ((alias ("lFifoFillAll")));
int32_t fifo_write_string(fifo_t *fifo, const uint8_t *string) __attribute__ ((alias ("lFifoWriteString")));
int32_t fifo_write_string_all(fifo_t *fifo, uint8_t *string) __attribute__ ((alias ("lFifoWriteStringAll")));
uint8_t fifo_write_byte(fifo_t *fifo, uint8_t data) __attribute__ ((alias ("bFifoWriteByte")));
int32_t fifo_read(fifo_t *fifo, uint8_t *out_buf, uint16_t count) __attribute__ ((alias ("lFifoRead")));
int32_t fifo_peek(fifo_t *fifo, uint8_t *out_buf, uint16_t count) __attribute__ ((alias ("lFifoPeek")));
int32_t fifo_shift(fifo_t *fifo, uint16_t count) __attribute__ ((alias ("lFifoShift")));
void fifo_flush(fifo_t *fifo) __attribute__ ((alias ("vFifoFlush")));
uint8_t fifo_read_byte(fifo_t *fifo, uint8_t* out_data) __attribute__ ((alias ("bFifoReadByte")));
uint8_t fifo_redirect(fifo_t *from, fifo_t *to) __attribute__ ((alias ("bFifoRedirect")));

uint8_t fifo_transaction_begin(fifo_t *fifo) __attribute__ ((alias ("bFifoTransactionBegin")));
uint8_t fifo_transaction_commit(fifo_t *fifo) __attribute__ ((alias ("bFifoTransactionCommit")));
uint8_t fifo_transaction_rollback(fifo_t *fifo) __attribute__ ((alias ("bFifoTransactionRollback")));
