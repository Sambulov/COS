...
static uint8_t _uspd_expander_handler(coroutine_t *this, uint8_t cancel, void *arg) {
...
hdl_module_state_t bldl_uspd_port_expander(void *desc, uint8_t enable) {
  static coroutine_t expander_task;
  if(enable) {
    coroutine_add(&expander_task, &_uspd_expander_handler, desc);
    return HDL_MODULE_ACTIVE;
  }
