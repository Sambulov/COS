...
typedef enum {
  HDL_EXTI_SOURCE_PA = EXTI_SOURCE_GPIOA,
  HDL_EXTI_SOURCE_PB = EXTI_SOURCE_GPIOB,
  HDL_EXTI_SOURCE_PC = EXTI_SOURCE_GPIOC,
  HDL_EXTI_SOURCE_PF = EXTI_SOURCE_GPIOF,
} hdl_exti_source_t;
...
uint8_t hdl_nvic_irq_request(hdl_interrupt_controller_t *ic, hdl_nvic_irq_n_t interrupt, event_handler_t handler, void *context);
static inline void hdl_nvic_sw_trigger(hdl_nvic_irq_n_t interrupt) {
