...
void hdl_event_unsubscribe(hdl_event_t *event, hdl_delegate_t *delegate) {
  hdl_event_private_t *evt = (hdl_event_private_t *)event;
  if(linked_list_contains(evt->delegates, linked_list_item((hdl_delegate_private_t*)delegate)))
...
void hdl_event_raise(hdl_event_t *event, void *sender, uint32_t event_trigger) {
  hdl_event_private_t *evt = (hdl_event_private_t *)event;
  if(event != NULL)
    linked_list_do_foreach(evt->delegates, &_call_delegate, (void *)((void*[]){&event_trigger, sender}));
}
