...
void exti0_1_handler()               { uint32_t pd = EXTI_PD & (EXTI_0 | EXTI_1); call_isr((hdl_nvic_irq_n_t)EXTI0_1_IRQn, (void *)pd); EXTI_PD |= pd; }
void exti2_3_IRQHandler()            { uint32_t pd = EXTI_PD & (EXTI_2 | EXTI_3); call_isr((hdl_nvic_irq_n_t)EXTI2_3_IRQn, (void *)pd); EXTI_PD |= pd; }
void exti4_15_handler()              { uint32_t pd = EXTI_PD & (EXTI_LINES_4_15); call_isr((hdl_nvic_irq_n_t)EXTI4_15_IRQn, (void *)pd); EXTI_PD |= pd; }
