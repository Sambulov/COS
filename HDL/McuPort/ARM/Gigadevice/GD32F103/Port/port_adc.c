  ...
  if(enable) {
    switch (hdl_adc->private.state_machine){
      case GD_ADC_STATE_MACHINE_INITIAL: {
        ...
        adc_special_function_config((uint32_t)hdl_adc->module.reg, ADC_CONTINUOUS_MODE, DISABLE);
        ...
        if(adc_source != NULL) {
          while (*adc_source != NULL) {
            adc_regular_channel_config((uint32_t)hdl_adc->module.reg, hdl_adc->private.channels_count, (uint8_t)(*adc_source)->channel, (uint32_t)(*adc_source)->sample_time);
            adc_source++;
            hdl_adc->config->values[hdl_adc->private.channels_count++] = HDL_ADC_INVALID_VALUE;
          }
        }
        adc_channel_length_config((uint32_t)hdl_adc->module.reg, ADC_REGULAR_CHANNEL, hdl_adc->private.channels_count);
      ...
      case GD_ADC_STATE_MACHINE_RUN:
        ...
        hdl_adc->private.age = 0;
        hdl_adc->private.start_conversion.context = desc;
...
uint32_t hdl_adc_age(hdl_adc_t *hdl_adc) {
  if((hdl_adc != NULL) && (hdl_state(&hdl_adc->module) == HDL_MODULE_ACTIVE)) {
    return ((hdl_adc_private_t *)hdl_adc)->private.age;
  }
  return 0;
}
