...
static uint8_t _hdl_tick_counter_get(const void *counter, uint32_t *value, uint32_t *period) {
  ...
  timer_interrupt_flag_clear(periph, TIMER_INT_FLAG_UP);
  if(value != NULL) {
    if(tick_cnt->config->counterdirection == TIMER_COUNTER_UP) *value = TIMER_CNT(periph);
    else *value = (TIMER_CAR(periph) - TIMER_CNT(periph));
  }
  if(period != NULL) *period = TIMER_CAR(periph);
  timer_enable(periph);
  return HDL_TRUE;
}
...
static uint8_t _hdl_tick_counter_set(const void *counter, uint32_t *value, uint32_t *period) {
  ...
  timer_interrupt_flag_clear(periph, TIMER_INT_FLAG_UP);
  if(value != NULL) {
    if(tick_cnt->config->counterdirection == TIMER_COUNTER_UP) TIMER_CNT(periph) = *value;
    else TIMER_CNT(periph) = (TIMER_CAR(periph) - *value);
  }
  if(period != NULL) TIMER_CAR(periph) = *period;
  timer_enable(periph);
  return HDL_TRUE;
}

static uint8_t _hdl_tick_counter_stop(const void *counter) {
  if(hdl_state(counter) == HDL_MODULE_FAULT) return HDL_FALSE;
  hdl_timertick_counter_t *tick_cnt = (hdl_timertick_counter_t *)counter;
  uint32_t periph = tick_cnt->config->phy;
  timer_disable(periph);
  timer_interrupt_flag_clear(periph, TIMER_INT_FLAG_UP);
