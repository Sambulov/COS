...
static uint8_t _nvm_flash_worker(coroutine_t *this, uint8_t cancel, void *arg) {
  ...
  if(message != NULL) {
    uint32_t address = flash->config->address_start + message->address + message->out_transferred;
    ...
    if(flash_var->state == FLASH_STATE_INITIAL) {
      ...
      if ((address + message->size) >= flash_end) {
        message->out_status |= HDL_NVM_ERROR_OUT_OF_RANGE;
      } 
      ...
      else if((message->options & HDL_NVM_OPTION_READ) && (message->rx_buffer != NULL)) {
        flash_var->state = FLASH_STATE_READ;
      }
      else if((message->options & HDL_NVM_OPTION_WRITE) && (message->tx_data != NULL)) { 
    ...
    if(flash_var->state == FLASH_STATE_WRITE) {
      ...
      if((message->out_transferred + transfer) > message->size)
        transfer = (message->size - message->out_transferred);
      if(!transfer) flash_var->state = FLASH_STATE_COMPETE;
      else {
        uint32_t fd = 0xffffffff;
        if(transfer != 4) {
          for (uint32_t i = byte_no; i < (byte_no + transfer); i++) {
            uint8_t invb = ~message->tx_data[message->out_transferred++];
            fd ^= (invb << (8 * i));
          }
        }
        else {
          fd = *((uint32_t *)(&message->tx_data[message->out_transferred]));
          message->out_transferred += 4;
        }
    ...
    if(flash_var->state == FLASH_STATE_ERASE) {
      ...
      if(message->out_transferred >= message->size) flash_var->state = FLASH_STATE_COMPETE;
      else {
        FMC->ISPCMD = FMC_ISPCMD_PAGE_ERASE;
        FMC->ISPADDR = address;
        FMC->ISPTRG = FMC_ISPTRG_ISPGO_Msk;
        message->out_transferred += FMC_FLASH_PAGE_SIZE;
      }
    }
    ...
    if(flash_var->state == FLASH_STATE_READ) {
      ...
      for (uint32_t i = 0; i < message->size; i++) {
        message->rx_buffer[i] = ((*ptr) >> (8 * byte_no));
        if(++byte_no >= 4) {
          byte_no = 0;
          ptr++;
        }
    ...
    if(flash_var->state == FLASH_STATE_COMPETE) {
      message->out_status |= HDL_NVM_STATE_COMPLETE;
...
static hdl_module_state_t _hdl_nvm(const void *desc, uint8_t enable) {
  ...
  if(flash_var->message != NULL) {
    flash_var->message->out_status |= HDL_NVM_ERROR_INTERNAL_FAULT;
  }
