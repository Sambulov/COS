...
{
    (void)u32ChannelNum; (void)u32Condition;
...
void EPWM_EnableDeadZone(EPWM_T *epwm, uint32_t u32ChannelNum, uint32_t u32Duration)
{
    ...
    (epwm)->DTCTL |= (EPWM_DTCTL_RDTEN0_Msk << ((u32ChannelNum) >> 1U)) | (EPWM_DTCTL_FDTEN0_Msk << ((u32ChannelNum) >> 1U));
    (epwm)->RDTCNT[(u32ChannelNum) >> 1U] = u32Duration;
    (epwm)->FDTCNT[(u32ChannelNum) >> 1U] = u32Duration;
}
