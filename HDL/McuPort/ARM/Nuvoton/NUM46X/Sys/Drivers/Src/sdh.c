...
void SDH_Set_clock(SDH_T *sdh, uint32_t sd_clock_khz)
{
    ...
    else
    {
        ...
        if (sdh == SDH0)
        {
            ...
            else if(u32SD_ClkSrc == CLK_CLKSEL0_SDH0SEL_PLL_DIV2)
            {
                _SDH0_ReferenceClock = ((CLK_GetPLLClockFreq()>>1) / 1000ul);
            }
        ...
        else
        {
            ...
            else if(u32SD_ClkSrc == CLK_CLKSEL0_SDH1SEL_PLL_DIV2)
            {
                _SDH1_ReferenceClock = ((CLK_GetPLLClockFreq()>>1) / 1000ul);
            }
