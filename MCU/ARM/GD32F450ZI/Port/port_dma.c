...
hdl_dma_status_e hdl_dma_status(hdl_dma_channel_t *channel) {
  ...
  if(dma != NULL) {
    ...
    if( dma_flag_get((uint32_t)dma->module.reg, dma_ch_no(channel), DMA_FLAG_FEE) == RESET || 
          dma_flag_get((uint32_t)dma->module.reg, dma_ch_no(channel), DMA_FLAG_SDE) == RESET ||
            dma_flag_get((uint32_t)dma->module.reg, dma_ch_no(channel), DMA_FLAG_TAE) == RESET) 
