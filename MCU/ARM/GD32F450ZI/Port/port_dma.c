...
static const uint32_t gd_dma_peripheral_width_array[] = {DMA_PERIPH_WIDTH_8BIT, DMA_PERIPH_WIDTH_16BIT, DMA_PERIPH_WIDTH_32BIT};
...
uint8_t hdl_dma_config(hdl_dma_t *dma, hdl_dma_config_t *config, hdl_dma_channel_t channel) {
  ...
  dma_memory_address_config(dma_periph, channel, DMA_MEMORY_0, config->memory_addr);
  ...
  if(config->memory_inc)
    dma_memory_address_generation_config(dma_periph, channel, DMA_MEMORY_INCREASE_ENABLE);
  else 
    dma_memory_address_generation_config(dma_periph, channel, DMA_MEMORY_INCREASE_DISABLE);

  if(config->periph_inc)
    dma_peripheral_address_generation_config(dma_periph, channel, DMA_PERIPH_INCREASE_ENABLE);
  else
    dma_peripheral_address_generation_config(dma_periph, channel, DMA_PERIPH_INCREASE_DISABLE);
  
  if(config->direction == HDL_DMA_DIRECTION_P2M)
      dma_transfer_direction_config(dma_periph, channel, DMA_PERIPH_TO_MEMORY);
  else if(config->direction == HDL_DMA_DIRECTION_M2P)
      dma_transfer_direction_config(dma_periph, channel, DMA_MEMORY_TO_PERIPH);
...
hdl_dma_status_e hdl_dma_status(hdl_dma_t *dma, hdl_dma_channel_t channel){
  uint32_t dma_periph = (uint32_t)dma->module.reg;
    ...
    if(dma_flag_get(dma_periph, channel, DMA_FLAG_FEE) == RESET)
