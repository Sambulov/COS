...
EmbeddedCli *embeddedCliNew(EmbeddedCliConfig *config) {
    ...
    if (config->cliBuffer == NULL) {
        config->cliBuffer = (CLI_UINT *) cli_malloc(totalSize); // malloc guarantees alignment.
        if (config->cliBuffer == NULL)
            return NULL;
        allocated = true;
...
embedded_cli_config_t *embedded_cli_default_config(void)           __attribute__ ((alias ("embeddedCliDefaultConfig")));
uint16_t embedded_cli_required_size(embedded_cli_config_t *config) __attribute__ ((alias ("embeddedCliRequiredSize")));
...
void embedded_cli_print(embedded_cli_t *cli, const char *string)   __attribute__ ((alias ("embeddedCliPrint")));
void embedded_cli_free(embedded_cli_t *cli)                        __attribute__ ((alias ("embeddedCliFree")));
void embedded_cli_tokenize_args(char *args)                        __attribute__ ((alias ("embeddedCliTokenizeArgs")));
const char *embedded_cli_get_token(const char *tokenized_str, uint16_t pos) 
                                                                   __attribute__ ((alias ("embeddedCliGetToken")));
char *embedded_cli_get_token_variable(char *tokenized_str, uint16_t pos) 
                                                                   __attribute__ ((alias ("embeddedCliGetTokenVariable")));
uint16_t embedded_cli_find_token(const char *tokenized_str, const char *token) 
                                                                   __attribute__ ((alias ("embeddedCliFindToken")));
uint16_t embedded_cli_get_token_count(const char *tokenized_str)   __attribute__ ((alias ("embeddedCliGetTokenCount")));
